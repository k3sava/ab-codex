...
slug: mihika-kapoor
roles:
  - Product manager (design-engineering hybrid), Figma
  - Early PM, FigJam
...
- **Vision is built cross-functionally, not handed off.** Research, design, engineering pollinate from the start.
- **Interleaved proof in pitches.** Pain → solution → proof per beat, not three sequential acts.
- **Confidence labels.** Strong opinions need explicit high / medium / low so the room knows when to push back.
